...
Besides this main control concern, I was having some other new insights I would like to keep in mind which are listed in section 2 and 3.
These final parts ended maybe a bit more towards a new structure for my thesis._

### 1 don't just ignore the influence of a bad attitude controller

```Damn you, attitude control and system dynamics!```

One of the main aspects of the problem I was disregarding was the difficulty and influence of the attitude controller as well as the dynamics of the drone.
...
You'll see that if the network has some temporal aspect for instance with an LSTM or 3D-CNN, the neural network will learn to behave towards this specific controller. 
...
This means closing the control or range shift, which is the shift on the influence of the output signal. This shift is not present when you train on a dataset because the prediction/decision has no impact on the environment and so the next state your agent is in.

An alternative approach could be the use of randomization or generalization over the aspects in simulation of which you are uncertain that they model te real world. 
I partially played around with this by adding translational and orientational noise to the drone so it would act weird in the hope of covering the rarities of the attitude control and the system dynamics. 
In this setting, the DNN need to learn to find the safest control while these strange turbulence apply.
...
It is however unclear how we best deal with this problem and whether it is a problem that should be covered in my thesis. 

If we want to **ignore the problem** or at least identify the problem we should at least run the doshico experiment with a floating camera object that acts perfectly stable. Or we can test it with a turtlebot. 
Alternatively we could try to **generalize** over the different control parameters as defined above.
A final and most attractive strategy might be to **learn to adapt** to the dynamics of the control in an unsupervised fashion. For this last strategy there are a number of interesting tools to use:
...
* If the only issue is drift, I could augment the controller with a control mapping that compensates in roll for current turn. This however requires a proper velocity estimation that needs to be learned. I'm referring here to Bart Theys's Phd. Training egomotion is an important robotics-vision task that might be worth exploring.

Though both methods seem valid, they both are not specially exciting research directions for my phd. So if it really comes to this I might try one of the two out, assuming I already have a nice working DNN ready to perform a task in the real world on for instance a turtlebot. So in that sense I would prefer to choose to ignore the problem for now and work with the turtlebot or a drone with zero inertia.
...
Some more difficult tasks would for instance be a hide-and-seek implementation where a drone should learn to scan an area in search for a target. This task includes remembering what areas are already screened, what parts of the scenery is occluded so should be discovered further, direct a path once the object is in view, ... . Whether the AI learns to do a depth-first search or screens areas more thoroughly on the fly, is open for the AI to learn. 
From a game point of view, it would be cool to make the target a separate AI that learns to hide as an adversary from the searching AI.

It will be important to start off from a high abstraction level, very game-like, to shape the reward, to see the influence of imitation learning, ... .
Maybe a gridworld example with a full state-space can already be interesting to start from and apply TRPO-GAE to or another state-of-the-art RL algorithm for discrete action space. The fun comes in when the adversarial agent is learned simultaneously. 
...
Once we are capable of training 2 agents on a partially observable environment, it is time to go to a turtlebot in an empty room with one red object that is the target to get to as a goal state in simulation. 

In order to simplify the training procedure you could add the current goal location as input of which gradually noise is added so the networks learns to ignore it. Although this already makes an assumption that a network is capable of forgetting in a stable manner.
...
* a subnetwork for obstacle avoidance (which can be learned separately)
* a meta network should than combine the different outputs as well as feature representations to learn which part of the controller becomes most important/reliable.
...
I am however very sceptible in how all this is reasonable to implement in less than 1 year.

In order to dive deeper into to benefits of having everything learned and approximated with deep neural networks, 
I could make the same thing with SLAM and see which one can find the goal faster, resulting in 2 algorithms competing in a search mission.

* obstacle avoidance < SLAM-based path-planning
* screening indoor scene < SLAM-based discovering (adding uncertainty in decision on target)
...
Again, probably smarter implementations will be required to combine the information from the target detector with the uncertainty-map builder and general obstacle avoidance.


On the other hand, it is discussible where the real benefits of end-to-end learning lies. Of course it would be interesting to research the real benefits of having a neural network learned to act in a similar fashion. 
...
* competing obstacle avoidance (driving at fixed speed) with SLAM + octomap + path-planning versus training it end-to-end with a DNN: pros and cons on both robustness, tweaking, hardware constraints. It seems a bit like comparing 2 different houses that are so different that it seems like compare epples with pears. By which I mean, you can only fall back on qualitative results mainly based on how it is implemented, more than on the algorithm itself. The real research question lies in what are the exact strengths of using universal functions approximators over predifined dynamic models.
* visualize learned features for OA, discovering as well as target capturing might enlighten robotics community in the relevance of these features in contrast with having a labelled octomap. Using these learned features / behaviors could than again enforce the handcratfted agent.
* potential of encapsulating behaviors in a learning fashion which is hard to define from an optimalization strategy: a proof of concept

The main critic in my point of view is that everything separately has already been shown to work so there is not much novelty. Although I don't think anyone has tried to have it all combined in a finished robotic solution of having 2 robots chasing eachother.

It's gonna be a hell of a work to implement. Therefore it must be very clear what the exact goals and research questions are and how they can be shown in a reliable manner. The latter remains an open question though.

This is kind of as good as state-of-the-art learning has shown to perform in real-world robotic applications. 
It is however unfortunate if we can't finish this or if the reinforced agent training does not seem to work at all.

The main contribution I see is the search for structural priors (architecture, unsupervised pretraining, auxiliary tasks, ? use of basis filters ?) to simplify the learning on a set of different tasks. On the other hand seems the integration of a slam-based hide-and-seek robot a nice robotics application although it has no novelty. Hide-and-seek agents trained with DRL is also an interesting proof-of-concept but again no research message there.
...
Another big downside is the lack of real-world experiments or demonstrations, unless for the handcrafted algorithm.
...
General research question could be: what are the strongest features to use for indoor collision avoidance with drones given both a perceptual domain and a control range shift. Ok, maybe without the control range shift.
...
A paper-concept could be: 'how naive imagenet-features are not interesting for autonomous navigation.' 
...
However I do see the value of having an example flight for instance from which features could be finetuned as well as control-dynamics(?).

Sample efficiency should be improved by using the structural priors. But wether the agent should learn with an A3C or TRPO or model based is not a main concern of this thesis. I just hope it learns something within a reasonable amount of time.
...
But at least it will fit nicely in a step-by-step project with clear intermediate hopefully success stories. Right?
...
Because it is SLAM based this should be able to work directly in the real-world given I have a good enough laptop (with dongle), stable enough drone and clear flying area.
